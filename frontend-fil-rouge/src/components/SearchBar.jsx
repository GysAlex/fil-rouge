...
export function SearchBar() {
    ...
    const handleSearch = () => {
        if (selectedRegion || selectedUniversity) {
            ...
            navigate(`/results/${selectedRegion || 'all'}/${selectedUniversity || 'all'}`);
        }
        toast.info("Veillez choisir une région ou une université pour effectuer la recherche")
    };
    ...
    return (
        <>
            <div className="bg-white hidden z-20 search relative py-5 px-10 h-fit min-h-[80px] gap-[30px] rounded-full md:flex items-center justify-center" 
                ...
                <Button handleClick={handleSearch} disabled={loading}>
